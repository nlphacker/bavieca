#! /usr/bin/perl

use warnings;
use strict;
use Time::HiRes qw( usleep ualarm gettimeofday tv_interval );
use Fcntl;
use File::Basename;

use FindBin;                                  
use lib "$FindBin::Bin/../scripts";  # use the scripts directory
use BaviecaLattices;

#------------------------------------------------------------------- 
# Daniel Bolanos 2011
# Boulder Language Technologies / University of Colorado at Boulder
#
# description: attach lm log-likelihoods to edges in the lattices
#
#-------------------------------------------------------------------

# input parameters
my ($filePhoneSet,$fileLexicon,$fileLM,$ip,$ipFiller,$dirLattices,$dirLatticesLM) = @ARGV;

# create the directory to keep aligned lattices
system("mkdir -p \"$dirLatticesLM\"");

# create auxiliar directories
system("mkdir -p \"$dirLatticesLM/lattices\"");
system("mkdir -p \"$dirLatticesLM/temp\"");

# process lattices session by session
opendir(FOLDER_SESSIONS, $dirLattices) || die("Could not open folder: $dirLattices");
my @data = grep(/[^\.]+$/,readdir(FOLDER_SESSIONS));
foreach my $dirSession (sort @data) {

	# semaphore for exclusive processing of sessions 
	my $fileTemp = "$dirLatticesLM/temp/$dirSession\.tmp";
  	if (! defined(sysopen(FILE, $fileTemp, O_WRONLY|O_CREAT|O_EXCL))) {
  		next;
  	}
  	close(FILE);

	BaviecaLattices::lmMarking("$dirLattices/$dirSession",
		"$dirLatticesLM/lattices/$dirSession",$filePhoneSet,$fileLexicon,$fileLM,$ip,$ipFiller);
	
}
closedir(FOLDER_SESSIONS);






