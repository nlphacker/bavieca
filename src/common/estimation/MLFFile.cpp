...
// constructor
MLFFile::MLFFile(LexiconManager *lexiconManager, const char *strFile, const unsigned char iMode)
{
...
void MLFFile::load() {
   ...
   while(std::getline(file.getStream(),strLine)) {
   	...
   	// word?
   		...
   		if (mlfUtterance == NULL) {
   			BVC_ERROR << "loading MLF at line " << m_strFile << " at line " << iLine << ", unexpected lexical unit found or wrong format of feature file name";
   		}
...
bool MLFFile::isFeatureFilename(string &strFile) {
	...
	if ((strFile.find("\"") != 0) ||
		(strFile.find(".fea\"") != strFile.length()-5)) {
		return false;	
	}
...
LexUnit *MLFFile::parseLexUnitLine(string &strLexUnit, int iMLFLine) {
   ...
   size_t iFirst = strLexUnit.rfind("(");
	size_t iLast = strLexUnit.rfind(")");
   if ((iFirst != strLexUnit.npos) && (iLast != strLexUnit.npos)) {
		if (iLast-iFirst < 2) {
			BVC_ERROR << "invalid lexical unit \"" << strLexUnit << "\" read from the MLF: line " << iMLFLine;
		}
	...
	if (lexUnit == NULL) {
		BVC_ERROR << "lexical unit \"" << strLexUnit << "\" read from the MLF was not found in the lexicon: line " << iMLFLine;
	}
