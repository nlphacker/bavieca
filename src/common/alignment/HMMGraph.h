...
typedef struct _FBEdgeHMM {
	...
	VAccumulator vAccumulator;			// acumulator (physical/logical)
	int iActive;
	int iEdge;
...
class HMMGraph {
	...
	private:
		...
		inline FBEdgeHMM *newFBEdgeHMM(FBNodeHMM *nodePrev, FBNodeHMM *nodeNext, HMMState *hmmStateEstimation, 
			HMMState *hmmStateUpdate, VAccumulator &vAccumulator, LexUnit *lexUnit, unsigned char iPosition) {
			...
			edge->vAccumulator = vAccumulator;
			assert(edge->vAccumulator.size() == vAccumulator.size());
			edge->lexUnit = lexUnit;
		...
		// check whether two sets of accumulators are equal
		inline bool equal(VAccumulator &vAccumulator1, VAccumulator &vAccumulator2) {
		
			if (vAccumulator1.size() != vAccumulator2.size()) {
				return false;
			}
			for(unsigned int i=0 ; i < vAccumulator1.size() ; ++i) {
				if (vAccumulator1[i] != vAccumulator2[i]) {
					return false;
				}
			}
		...
		inline bool equal(FBEdgeHMM *edge1, FBEdgeHMM *edge2) {
			...
			if (m_hmmManagerUpdate->areAccumulatorsLogical()) {
				...
				return ((edge1->hmmStateEstimation == edge2->hmmStateEstimation) && 
					(equal(edge1->vAccumulator,edge2->vAccumulator)));	
			} 
