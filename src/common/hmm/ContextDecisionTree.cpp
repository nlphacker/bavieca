...
ContextDecisionTree::ContextDecisionTree(int iFeatureDimensionality, int iCovarianceModelling, PhoneSet *phoneSet, unsigned char iContextModelingOrder, Accumulator *accumulator, Rule **rules, int iRules) {
	...
	m_vMeanCluster = new Vector<double>(m_iDim);
	m_vCovarianceCluster = new Vector<double>(m_iDim);
	m_vObservationCluster = new Vector<double>(m_iDim);
	m_vObservationSquareCluster = new Vector<double>(m_iDim);
	
	m_vMeanClusterNo = new Vector<double>(m_iDim);
	m_vCovarianceClusterNo = new Vector<double>(m_iDim);
	m_vObservationClusterNo = new Vector<double>(m_iDim);
	m_vObservationSquareClusterNo = new Vector<double>(m_iDim);
}
...
ContextDecisionTree::ContextDecisionTree(ContextDecisionTree *contextDecisionTreeSeed) {
	...
	m_iDim = contextDecisionTreeSeed->m_iDim;
...
ContextDecisionTree::~ContextDecisionTree() {
	...
	// release memory
	if (m_bEstimationDataAllocated) {
	
		delete m_vMeanCluster;
		delete m_vCovarianceCluster;
		delete m_vObservationCluster;
		delete m_vObservationSquareCluster;
		
		delete m_vMeanClusterNo;
		delete m_vCovarianceClusterNo;
		delete m_vObservationClusterNo;
		delete m_vObservationSquareClusterNo;	
	}
}
...
void ContextDecisionTree::clusterNode(DTNode *node, float fMinimumClusterOccupation, float fMinimumLikelihoodGainClustering) {
	...
	for(int iRule = 0 ; iRule < node->iRules ; ++iRule) {
		...
		/*bool b = computeLikelihoodRule(node,node->rules[iRule],&fLikelihoodRuleYes,&fLikelihoodRuleNo,&fOccupationRuleYes,&fOccupationRuleNo,fMinimumClusterOccupation,fCovarianceGlobal);
		if (b == false) {
			continue;
		}*/
...
void ContextDecisionTree::computeLikelihoodCluster(DTNode *node, double *dLikelihoodCluster, double *dOccupationCluster, Vector<float> &vCovarianceGlobal) {
	...
	m_vObservationCluster->zero();
	m_vObservationSquareCluster->zero();
	...
	// (1) compute the mean
	m_vMeanCluster->mul(1.0/m_dOccupationCluster,*m_vObservationCluster);
	
	// (2) compute the covariance (using the already computed mean)
	for(int i=0 ; i < m_iDim ; ++i) {
		(*m_vCovarianceCluster)(i) = ((m_dOccupationCluster*(*m_vMeanCluster)(i)*(*m_vMeanCluster)(i))+(*m_vObservationSquareCluster)(i)-
		(2*(*m_vObservationCluster)(i)*(*m_vMeanCluster)(i)))/m_dOccupationCluster;	
	}
	
	// (3) compute the likelihood of the new cluster
	assert(m_vCovarianceCluster->isPositive());
	double dDeterminant = m_vCovarianceCluster->sumLog();
	assert(finite(dDeterminant));
	*dLikelihoodCluster = ((-0.5*m_dOccupationCluster)*(dDeterminant+(m_iDim*(log(2.0*PI_NUMBER)+1.0))));	
...
double ContextDecisionTree::computeLikelihoodCluster(double dOccupation, Vector<double> &vCovariance) {
	...
	double dDeterminant = vCovariance.sumLog();
	assert(finite(dDeterminant));
	double dLikelihood = -0.5*dOccupation*(dDeterminant+(m_iDim*(log(2.0*PI_NUMBER)+1.0)));	
...
bool ContextDecisionTree::computeLikelihoodRule(DTNode *node, Rule *rule, bool bAnswer, double *dLikelihoodCluster, double *dOccupationCluster, float fMinimumClusterOccupation) {
	...
	*dOccupationCluster = m_dOccupationCluster;
	...
	m_vMeanCluster->mul(1.0/m_dOccupationCluster,*m_vObservationCluster);	
	m_vCovarianceCluster->mul(1.0/m_dOccupationCluster,*m_vObservationSquareCluster);
	m_vCovarianceCluster->addSquare(-1.0,*m_vMeanCluster);
	
	// (3) compute the likelihood of the new cluster
	*dLikelihoodCluster = computeLikelihoodCluster(m_dOccupationCluster,*m_vCovarianceCluster);
...
bool ContextDecisionTree::computeLikelihoodRule(DTNode *node, Rule *rule, double *dLikelihoodClusterYes, double *dLikelihoodClusterNo, double *dOccupationClusterYes, double *dOccupationClusterNo, float fMinimumClusterOccupation) {
	...
	m_vObservationCluster->zero();
	m_vObservationSquareCluster->zero();
	m_vObservationClusterNo->zero();
	m_vObservationSquareClusterNo->zero();
	m_dOccupationCluster = 0.0;
	...
	while(accumulator != NULL) {	
		...
		// negative answer
			...
			m_vObservationClusterNo->add(accumulator->getObservation());
			m_vObservationSquareClusterNo->add(accumulator->getObservationSquareDiag());
	...
	*dOccupationClusterNo = m_dOccupationClusterNo;
	...
	if ((m_dOccupationCluster < fMinimumClusterOccupation) || 
		(m_dOccupationClusterNo < fMinimumClusterOccupation)) {
		return false;
	}	
	...
	// (1) compute the mean
	m_vMeanCluster->mul(1.0/m_dOccupationCluster,*m_vObservationCluster);
	m_vMeanClusterNo->mul(1.0/m_dOccupationClusterNo,*m_vObservationClusterNo);
	
	// (2) compute the covariance (using the already computed mean)
	m_vCovarianceCluster->mul(1.0/m_dOccupationCluster,*m_vObservationSquareCluster);
	m_vCovarianceCluster->addSquare(-1.0,*m_vMeanCluster);
	
	m_vCovarianceClusterNo->mul(1.0/m_dOccupationClusterNo,*m_vObservationSquareClusterNo);
	m_vCovarianceClusterNo->addSquare(-1.0,*m_vMeanClusterNo);
		
	// (3) compute the likelihood of the new cluster
	*dLikelihoodClusterYes = computeLikelihoodCluster(m_dOccupationCluster,*m_vCovarianceCluster);
	*dLikelihoodClusterNo = computeLikelihoodCluster(m_dOccupationClusterNo,*m_vCovarianceClusterNo);
...
double ContextDecisionTree::computeLikelihoodFromMerging(DTNode *nodeA, DTNode *nodeB, Vector<float> &vCovarianceGlobal) {
	...
	// (3) compute the likelihood of the new cluster
	double dLikelihoodCluster = computeLikelihoodCluster(m_dOccupationCluster,*m_vCovarianceCluster);
...
int ContextDecisionTree::compactTreeLeaves(float fMinimumLikelihoodGainClustering, Vector<float> &vCovarianceGlobal) {
	...
	for(LDTNode::iterator it = lLeaves.begin() ; it != lLeaves.end() ; ) {
		...
		for(LDTNode::iterator jt = lLeaves.begin() ; jt != lLeaves.end() ; ++jt) {
			...
			double dLikelihoodCluster = computeLikelihoodFromMerging(*it,*jt,vCovarianceGlobal);
...
void ContextDecisionTree::createHMMStateNoData(unsigned char iPhone, unsigned char iState, HMMState **hmmStates, int *iLeavesSeen, int *iUnitsCreated, Vector<float> &vMeanGlobal, Vector<float> &vCovarianceGlobal) {
	
	// create the HMM-state	
	m_dtnodeRoot->iHMMState = *iUnitsCreated;
	hmmStates[*iUnitsCreated] = new HMMState(m_iDim,m_iCovarianceModelling,m_phoneSet,iPhone,iState,-1,1,*iUnitsCreated);
	Gaussian *gaussian = hmmStates[m_dtnodeRoot->iHMMState]->getMixture()(0);
	
	// default mean and covariance to global distribution
	gaussian->mean().copy(vMeanGlobal);
	gaussian->covarianceDiag().copy(vCovarianceGlobal);
	gaussian->weight() = 1.0;
	
	// sanity check (compute the likelihood of the HMM-state)
	double dDeterminant = 0.0;
	for(int i=0 ; i<m_iDim ; ++i) {
		dDeterminant += log(gaussian->covarianceDiag()(i));
	}
	assert(finite(dDeterminant));
...
void ContextDecisionTree::createClusteredContextDependentUnitsFromLeaves(DTNode *node, HMMState **hmmStates,
	int *iLeavesSeen, int *iUnitsCreated, double *dLikelihood) {

	// if it is not a leaf, continue
	if (node->rule != NULL) {
		createClusteredContextDependentUnitsFromLeaves(node->dtnodeYes,hmmStates,iLeavesSeen,iUnitsCreated,dLikelihood);
		createClusteredContextDependentUnitsFromLeaves(node->dtnodeNo,hmmStates,iLeavesSeen,iUnitsCreated,dLikelihood);
		return;
	}	
	// leaf node
	else {
		...
		if ((node->bIsMerged) && (node->iHMMState != -1)) {
			// increment the number of leaves and accumulate likelihood
			(*iLeavesSeen)++;	
			return;
		}
		...
		if (node->dtnodeMerged != NULL) {
			...
			// otherwise create it
				...
				gaussian = hmmStates[node->iHMMState]->getMixture()(0);	
			}	
		} else {
			// create the HMM-state	
			unsigned char iPhone = node->accumulator->getPhone();
		...
		// (3) compute the mean
		gaussian->mean().mul((float)(1.0/m_dOccupationCluster),*m_vObservationCluster);
		
		// (4) compute the covariance (using the already computed mean)
		gaussian->covarianceDiag().mul((float)(1.0/m_dOccupationCluster),*m_vObservationSquareCluster);
		...
		// sanity check (compute the likelihood of the HMM-state)
		double dLikelihoodCluster = computeLikelihoodCluster(m_dOccupationCluster,gaussian->covarianceDiag());
