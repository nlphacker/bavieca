...
class ContextDecisionTree {
	...
	private:
		...
		double *m_dMeanCluster;
		double *m_dCovarianceCluster;
		double m_dOccupationCluster;
		double *m_dObservationCluster;
		double *m_dObservationSquareCluster;
		
		double *m_dMeanClusterNo;
		double *m_dCovarianceClusterNo;
		double m_dOccupationClusterNo;
		double *m_dObservationClusterNo;
		double *m_dObservationSquareClusterNo;			
	...
	private:
		...
		void createClusteredContextDependentUnitsFromLeaves(DTNode *dtnode, HMMState **hmmStates, int *iLeaves, int *iUnitsCreated, double *dLikelihood, float fCovarianceFlooringRatio);
	...
	public:
		...
		bool computeLikelihoodCluster(DTNode *node, double *dLikelihoodCluster, double *dOccupationCluster, float fCovarianceFlooringRatio, float *fCovarianceGlobal);		
		
		// compute the likelihood of the root node
		inline bool computeLikelihoodRoot(double *dLikelihoodCluster, double *dOccupationCluster, float fCovarianceFlooringRatio, float *fCovarianceGlobal) {
		
			if (computeLikelihoodCluster(m_dtnodeRoot,dLikelihoodCluster,dOccupationCluster,
				fCovarianceFlooringRatio,fCovarianceGlobal) == true) {
		...
		double computeLikelihoodFromMerging(DTNode *nodeA, DTNode *nodeB, float fCovarianceFlooringRatio, float *fCovarianceGlobal);	
		...
		int compactTreeLeaves(float fMinimumLikelihoodGainClustering, float fCovarianceFlooringRatio, float *fCovarianceGlobal);
		
		// create the new set of HMM-states from the leaves of the tree (return the number of physical nphones created)
		inline void createClusteredContextDependentUnitsFromLeaves(HMMState **hmmStates, int *iLeavesSeen, int *iUnitsCreated, double *dLikelihood, float fCovarianceFlooringRatio) {
		
			return createClusteredContextDependentUnitsFromLeaves(m_dtnodeRoot,hmmStates,iLeavesSeen,iUnitsCreated,dLikelihood,fCovarianceFlooringRatio);
		}
		
		// create an HMM-state when there is no data associated to it (default to globla distribution)
		void createHMMStateNoData(unsigned char iPhone, unsigned char iState, HMMState **hmmStates, int *iLeavesSeen, int *iUnitsCreated, float *fMeanGlobal, float *fCovarianceGlobal);
		...
		double computeLikelihoodCluster(double dOccupation, double *dCovariance);
		
		// compute the cluster likelihood
		double computeLikelihoodCluster(double dOccupation, float *fCovariance);
