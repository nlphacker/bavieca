...
void PhoneSet::load() {
	...
	while(std::getline(file.getStream(),strLine).good()) {
		...
		if (strPhone.length() > MAX_PHONETIC_SYMBOL_LENGTH) {
			ERROR << "incorrect phonetic symbol name: " << strPhone << ", too many characters";
		}
		
		// check that the phone name is correct
		for(int i=0 ; i < strPhone.length() ; ++i) {
			if (!isalnum(strPhone.c_str()[i]) && (strPhone.c_str()[i] != '_')) {
				ERROR << "incorrect phonetic symbol name: " << strPhone << ", wrong symbol";
			}
		} 
   ...
   if (m_phones.empty()) {
   	ERROR << "no phonetic symbols found in the phonetic symbol file";
   }
   
   // check that the silence is defined
   if (m_mPhone.find(PHONETIC_SYMBOL_SILENCE) == m_mPhone.end()) {
   	ERROR << "phonetic symbol " << PHONETIC_SYMBOL_SILENCE << " must be defined in the phonetic symbol file";
   }	
}
