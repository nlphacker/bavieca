...
// constructor
WFSADecoder::WFSADecoder(ConfigurationDecoder *configurationDecoder, PhoneSet *phoneSet, HMMManager *hmmManager,
	LexiconManager *lexiconManager, WFSAcceptor *wfsAcceptor, Log *log, int iPruningMaxActiveStates, float fPruningLikelihood, bool bLatticeGeneration, int iMaxWordSequencesState) {

	m_configurationDecoder = configurationDecoder;
	m_phoneSet = phoneSet;	
...
bool WFSADecoder::initialize() {
	...
	if (m_activeStateTable->initialize() == false) {
		return false;
	}	
...
bool WFSADecoder::viterbi(float *fFeatureVectors, unsigned int iFeatureVectors) {
	...
	ActiveState *activeStatesNext = NULL;
