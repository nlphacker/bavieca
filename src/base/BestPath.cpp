...
void BestPath::print(bool bExtended) {
	...
	for( list<BestPathElement*>::iterator it = m_lBestPathElements.begin() ; it != m_lBestPathElements.end() ; ++it ) {	
		...
		if (bExtended) {
			cout << " (gl= " << setw(12) << (*it)->fScore << ") (am= " << setw(12) << (*it)->fScoreAcousticModel 
				<< ") (lm= " << setw(10) << (*it)->fScoreLanguageModel << ") (ip= " << setw(4) << (*it)->fInsertionPenalty 
...
void BestPath::write(ostream &os, const char *strAudioFile, const char *strSpeakerId, float fOffset, 
	...
	for( list<BestPathElement*>::iterator it = m_lBestPathElements.begin() ; it != m_lBestPathElements.end() ; ++it ) {
		...
		if (!bSentenceDelimiter) {
			fStart = fOffset + ((*it)->iFrameStart + m_iFrameStart) * 0.01; 
			fDuration = ((*it)->iFrameEnd - (*it)->iFrameStart + 1) * 0.01;
		}
...
bool BestPath::getLexUnits(VLexUnit &vLexUnit) {
