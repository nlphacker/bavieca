...
void PhoneticRulesManager::load() {
   ...
   int iBasePhones = m_phoneSet->getSize();
   int iLine = 1;
   ...
   while(std::getline(file.getStream(),strLine).good()) { 
      ...
      for(int i=0 ; i < iBasePhones+1 ; ++i) {
		...
		int iLength = strlen(strLineCopy);
      ...
      while(str != NULL) {
         if (str[0] != '$') {
         	...
         	if (iPhone == -1) {
         		WARNING << "unknown phonetic symbol \"" << str << "\" found in file: "
            ...
            if (aux == NULL) {
            	ERROR << "phonetic rule " << (str+1) << " used before but not defined yet (line: " << iLine << ")" << endl;
            }
            // copy the phones
   ...
   for(int i=0 ; i<iBasePhones+1 ; ++i) {
   	if (bPhone[i] == false) {
   		WARNING << "phonetic symbol " << m_phoneSet->getStrPhone(i) << " not represented by any rule";
   	}
   }
   delete [] bPhone;
}
...
void PhoneticRulesManager::store(FileOutput &file) {
   ...
   // (1) phonetic symbols
   int iPhones = m_phoneSet->getSize();
...
PhoneticRulesManager *PhoneticRulesManager::load(FileInput &file, PhoneSet *phoneSet) {
   ...
   assert(iPhones == phoneSet->getSize());
   char *strPhone = NULL;
   for(int i=0 ; i<iPhones ; ++i) {
