...
// constructor
ConfigurationWFSADecoder::ConfigurationWFSADecoder(const char *strFile) : ParameterManager(NULL),ConfigurationFile(strFile)
{
	m_strFile = strFile;
}
...
void ConfigurationWFSADecoder::defineParameters() {
	...
	// audio
	defineParameter("audio.format","audio format",PARAMETER_TYPE_STRING,true,"raw","raw");	
	defineParameter("audio.samplingRate","audio sampling rate (in kHz)",PARAMETER_TYPE_INTEGER,true,"8000|16000","16000");
	defineParameter("audio.sampleSize","audio sample size (in bits)",PARAMETER_TYPE_INTEGER,true,"16","16");
	...
	// logging 	
	defineParameter("log.file","log file",PARAMETER_TYPE_FILE,false);
	defineParameter("log.verbosityLevel","level of detail",PARAMETER_TYPE_STRING,false,"error|warning|information","warning");
...
void ConfigurationWFSADecoder::load() {
	...
	try {
		...
		ERROR << "unable to load the wfsa configuration file";
	}
}
