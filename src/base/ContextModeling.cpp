...
ContextModeling::ContextModeling(int iFeatureDimensionality, int iCovarianceModelling, PhoneSet *phoneSet, int iContextModelingOrderWW, int iContextModelingOrderCW, const char *strFilePhoneticRules, bool bGlobalClustering, bool bBottomUpMerging, float fMinimumClusterOccupation, float fMinimumLikelihoodGain, float *fMeanGlobal, float *fCovarianceGlobal, float fCovarianceFlooringRatio, const char *strFileDecisionTrees, Log *log)
{
	...
	m_fMeanGlobal = fMeanGlobal;
	m_fCovarianceGlobal = fCovarianceGlobal;
	m_fCovarianceFlooringRatio = fCovarianceFlooringRatio;
	m_iPhones = m_phoneSet->getSize();
	m_strFileDecisionTrees = strFileDecisionTrees;
	m_log = log;
...
bool ContextModeling::clusterContextDependentUnits(MAccumulatorLogical &mAccumulators, HMMManager *hmmManager) {
	...
	if (m_bGlobalClustering == false) {
		...
		float fLikelihoodAccumulated = 0.0;
		float fLikelihoodAccumulatedClustering = 0.0;
		int iLeavesAccumulated = 0;
		int iLeavesAccumulatedData = 0;
		int iLeavesAccumulatedWithoutData = 0;
		
		int iContextDecisionTrees = m_phoneSet->getSize()*NUMBER_HMM_STATES;
		ContextDecisionTree **contextDecisionTrees = new ContextDecisionTree*[iContextDecisionTrees];
		
		// for each state
		for(int iBasePhone = 0 ; iBasePhone < m_phoneSet->getSize() ; ++iBasePhone) {
			for(int iState = 0 ; iState < NUMBER_HMM_STATES ; ++iState) {
				...
				ContextDecisionTree *contextDecisionTree = new ContextDecisionTree(m_iFeatureDimensionality,
				...
				if (accumulator == NULL) {
					...
					sprintf(strMessage,"no data available to estimate the parameters of the HMM-state: %s(%d), defaulting to global distribution",m_phoneSet->getStrPhone(iBasePhone),iState);
					m_log->logWarning(strMessage);
					continue;
				}
				...
				if (contextDecisionTree->computeLikelihoodRoot(&dLikelihoodRoot,
					&dOccupationRoot,m_fCovarianceFlooringRatio,m_fCovarianceGlobal) == false) {
					return false;	
				}
				fLikelihoodAccumulated += dLikelihoodRoot;
					
				// clustering: only phones that need context dependency
				double dLikelihoodAfterClustering = dLikelihoodRoot;
				int iLeaves = 1;
				int iLeavesData = 1;
				if (m_phoneSet->isPhoneContextModeled(iBasePhone) == true) {
	
					// to make sure the user knows what is being done
					if ((iBasePhone == m_phoneSet->getPhoneIndex(PHONETIC_SYMBOL_SILENCE)) && (iState == 0)) {
						char strMessage[MAX_LOG_MESSAGE_LENGTH+1];
						sprintf(strMessage,"modeling context for phonetic symbol: %s",PHONETIC_SYMBOL_SILENCE);
						m_log->logWarning(strMessage);
					}
					...
					fLikelihoodAccumulatedClustering += dLikelihoodAfterClustering;
				...
				// do not cluster: for example SIL, breath, noise, etc
					...
					fLikelihoodAccumulatedClustering += dLikelihoodRoot;
					iLeavesAccumulated++;
					iLeavesAccumulatedData++;
				}
				float fPercentIncrease = 100.0*((dLikelihoodRoot-dLikelihoodAfterClustering)/dLikelihoodRoot);
				char strInformation[MAX_LOG_MESSAGE_LENGTH+1];
				sprintf(strInformation,"%s(%d) occ: %12.2f, likelihood: %16.4f -> %16.4f (%5.2f%%) %d %s",m_phoneSet->getStrPhone(iBasePhone),iState,dOccupationRoot,dLikelihoodRoot,dLikelihoodAfterClustering,fPercentIncrease,iLeavesData,Accumulator::getContextModelingOrder(m_iContextModelingOrderWW));
				m_log->logInformation(strInformation);
			}
		}
		
		assert(iLeavesAccumulated == (iLeavesAccumulatedData+iLeavesAccumulatedWithoutData));
		
		float fLikelihoodDecreasePercentClustering = (fLikelihoodAccumulatedClustering/fLikelihoodAccumulated)*100;
		int iPhysicalNphones = 0;
		
		printf("likelihood before clustering:   %20.4f %6.2f%% (%8d observed %s, %d monophones)\n",
			fLikelihoodAccumulated,100.0,m_iAccumulators,Accumulator::getContextModelingOrder(m_iContextModelingOrderWW),
			m_iPhones*NUMBER_HMM_STATES);	
		printf("likelihood top-down clustering: %20.4f %6.2f%% (%8d physical %s)\n",
			fLikelihoodAccumulatedClustering,fLikelihoodDecreasePercentClustering,iLeavesAccumulated,
		...
		if (m_bBottomUpMerging == true) {
			...
			float fLikelihoodAfterMerging = 0;
			for(int iHMM = 0 ; iHMM < m_iPhones*NUMBER_HMM_STATES ; ++iHMM) {
				...
				iLeavesAfterMerging += contextDecisionTrees[iHMM]->compactTreeLeaves(m_fMinimumLikelihoodGainClustering,
					m_fCovarianceFlooringRatio,m_fCovarianceGlobal);
				fLikelihoodAfterMerging += contextDecisionTrees[iHMM]->computeTreeLikelihood();
			}	
		
			float fLikelihoodDecreasePercentClusteringMerging = (fLikelihoodAfterMerging/fLikelihoodAccumulated)*100;
		
			printf("likelihood bottom-up merging:   %20.4f %6.2f%% (%8d physical %s)\n",
				fLikelihoodAfterMerging,fLikelihoodDecreasePercentClusteringMerging,iLeavesAfterMerging,Accumulator::getContextModelingOrder(m_iContextModelingOrderWW));
		...
		for(int iHMM = 0 ; iHMM < m_iPhones*NUMBER_HMM_STATES ; ++iHMM) {
			// if there is no data associated to the HMM-state continue
			if (contextDecisionTrees[iHMM]->getOccupation() == 0.0) {
				if ((m_fMeanGlobal == NULL) || (m_fCovarianceGlobal == NULL)) {
				...
				contextDecisionTrees[iHMM]->createHMMStateNoData(iPhoneAux,iStateAux,
					m_hmmStatesClustered,&iLeavesCheck,	&iNphonesCreated,m_fMeanGlobal,m_fCovarianceGlobal);
			...
			contextDecisionTrees[iHMM]->createClusteredContextDependentUnitsFromLeaves(m_hmmStatesClustered,&iLeavesCheck,
				&iNphonesCreated,&dLikelihoodCheck,m_fCovarianceFlooringRatio);
		...
		if (m_strFileDecisionTrees != NULL) {
			...
			if (storeContextDecisionTrees(m_strFileDecisionTrees,contextDecisionTrees,iContextDecisionTrees) == false) {
				char strMessage[MAX_LOG_MESSAGE_LENGTH+1];
				sprintf(strMessage,"unable to write the decision tree(s) to the file: %s",m_strFileDecisionTrees);
				m_log->logError(strMessage);
				return false;
			}
		}
	} 
	// global clustering: all the HMM-states are clustered together
		...
		float fLikelihoodClustering = 0.0;
		int iLeaves = 0;
		int iLeavesData = 0;		
		...
		if (accumulator == NULL) {
			char strMessage[MAX_LOG_MESSAGE_LENGTH+1];
			sprintf(strMessage,"no data available to carry out context dependent clustering");
			m_log->logError(strMessage);
			return false;
		}
		
		// create the context decision tree
		ContextDecisionTree *contextDecisionTree = new ContextDecisionTree(m_iFeatureDimensionality,m_iCovarianceModelling,m_phoneSet,m_iContextModelingOrderWW,
		...
		contextDecisionTree->clusterRoot(m_fMinimumClusterOccupation,m_fMinimumLikelihoodGainClustering);
		// get the likelihood from the leaves of the node
		fLikelihoodClustering += contextDecisionTree->computeTreeLikelihood();
		iLeaves = contextDecisionTree->countTreeLeaves();
		iLeavesData = contextDecisionTree->countTreeLeavesOccupancy();
		...
		float fLikelihoodDecreasePercentClustering = (fLikelihoodClustering/fLikelihood)*100;
		int iPhysicalNphones = 0;
...
bool ContextModeling::storeContextDecisionTrees(const char *strFile, ContextDecisionTree **contextDecisionTrees, int iContextDecisionTrees) {
...
ContextDecisionTree **ContextModeling::loadContextDecisionTrees(const char *strFile, int iFeatureDimensionality, int iCovarianceModelling, PhoneticRulesManager *phoneticRulesManager, int *iContextDecisionTrees, PhoneSet *phoneSet, unsigned char iContextModelingOrder, unsigned char iClusteringMethod) {
	...
	assert(iPhonesAux == phoneSet->getSize());
	...
	for(int i=0 ; i < *iContextDecisionTrees ; ++i) {
		...
		if (contextDecisionTrees[i] == NULL) {
			return NULL;
		}
	}
...
// log warnings
void ContextModeling::logWarning(const char *strMessage) {
...
// log errors
void ContextModeling::logError(const char *strMessage) {
...
void ContextModeling::computeGlobalDistribution(MAccumulatorLogical mAccumulators) {
	...
	int iCovarianceElements = HMMManager::getCovarianceElements(m_iFeatureDimensionality,m_iCovarianceModelling);

	// compute the global distribution
	double *dGlobalMean = new double[m_iFeatureDimensionality]; 
	double *dGlobalCovariance = new double[iCovarianceElements];
	double *dObservationSquare = new double[iCovarianceElements];
	
	m_fMeanGlobal = new float[m_iFeatureDimensionality]; 
	m_fCovarianceGlobal = new float[iCovarianceElements];	
	...
	for(MAccumulatorLogical::iterator it = mAccumulators.begin() ; it != mAccumulators.end() ; ++it) {	
		...
		for(int j = 0 ; j < m_iFeatureDimensionality ; ++j) {
         dGlobalMean[j] += accumulator->getObservation()[j]; 
		}
		dOccupationTotal += accumulator->getOccupation();
	}	 
	for(int i = 0 ; i < m_iFeatureDimensionality ; ++i) {
		dGlobalMean[i] /= dOccupationTotal;
	}
	...
	for(MAccumulatorLogical::iterator it = mAccumulators.begin() ; it != mAccumulators.end() ; ++it) {	
		Accumulator *accumulator = it->second;
		for(int i = 0 ; i < iCovarianceElements ; ++i) {
			dObservationSquare[i] += accumulator->getObservationSquare()[i];
		}
	}	
		
	// diagonal
	if (m_iCovarianceModelling == COVARIANCE_MODELLING_TYPE_DIAGONAL) {
		for(int i = 0 ; i < m_iFeatureDimensionality ; ++i) {
			dGlobalCovariance[i] = (dObservationSquare[i]/dOccupationTotal)-(dGlobalMean[i]*dGlobalMean[i]);
		}
	}	
	// full
	else {
		assert(m_iCovarianceModelling == COVARIANCE_MODELLING_TYPE_FULL);	
		int iIndex = 0;
		for(int i = 0 ; i < m_iFeatureDimensionality ; ++i) {
			for(int j = i ; j < m_iFeatureDimensionality ; ++j, ++iIndex) {
				dGlobalCovariance[iIndex] = (dObservationSquare[iIndex]/dOccupationTotal)-(dGlobalMean[i]*dGlobalMean[j]);
				if (i == j) {
					assert(dGlobalCovariance[iIndex] > 0.0);
				}
			}
		}
	}
	...
	for(int i = 0 ; i < m_iFeatureDimensionality ; ++i) {	
		m_fMeanGlobal[i] = dGlobalMean[i];
	}
	for(int i = 0 ; i < iCovarianceElements ; ++i) {	
		m_fCovarianceGlobal[i] = dGlobalCovariance[i];
	}
	
	delete [] dGlobalCovariance;
	delete [] dGlobalMean;
