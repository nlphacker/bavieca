...
typedef struct _LEdge {
   ...
   int iFrameEnd;								// end frame
	...
	// phonetic context
	unsigned char *iContextLeft;
	unsigned char *iContextRight;	
...
class HypothesisLattice {
	...
	public:
		...
		// constructor
		HypothesisLattice(PhoneSet *phoneSet, LexiconManager *lexiconManager, Log *log = NULL);
		...
		// set a property
		inline void setProperty(const char *strProperty, const char *strValue) {
		
			m_mProperties[strProperty] = strValue;
		}
		
		// set a property
		inline void setProperty(const char *strProperty, int iValue) {
		...
		// remove a property
		inline void removeProperty(const char *strProperty) {
		...
		inline bool isProperty(const char *strProperty) {
		
			return (m_mProperties.find(strProperty) != m_mProperties.end());
		}
		
		// check that a property has the given value
		inline bool checkProperty(const char *strProperty, const char *strValue) {
		...
		inline const char *getPropertyValue(const char *strProperty) {
		...
		static inline LNode *newNode(int iFrame) {
		...
		static inline LEdge *newEdge(int iFrameStart, int iFrameEnd, LexUnit *lexUnit, float fScoreAM, float fScoreLM) {
		...
		static inline LEdge *newEdge(LEdge *edgeOriginal) {
		...
		// connect an edge
		static inline void connectEdge(LNode *lnode1, LEdge *ledge, LNode *lnode2) {
		...
		void load(const char *strFile, unsigned char iFormat);	
		
		// load the lattice from the given file (text format)
		void loadTextFormat(const char *strFile);	
		...
		// by equivalent arcs
		inline bool equivalentSuccessors(LNode *node1, LNode *node2) {
		...
		// by equivalent arcs
		inline bool equivalentPredecessors(LNode *node1, LNode *node2) {
		...
		inline static bool compareEdgesOrder(const LEdge *edge1, const LEdge *edge2) {
		...
		inline static bool equivalentEdges(LEdge *ledge1, LEdge *ledge2) {
		...
		inline const char* getStrConfidenceMeasure(unsigned char iConfidenceMeasure) {
		...
		bool computeForwardBackwardScores(float fScalingAM, float fScalingLM);
		...
		bool computePosteriorProbabilities();
		
		// confidence estimation ---------------------------------------------------------------
		
		// compute posterior-probability based confidence estimates
		bool computeConfidenceScore(unsigned char iConfidenceMeasure);
		
		// return whether two edges overlap in time
		inline bool overlap(LEdge *edge1, LEdge *edge2) {
		...
		inline bool spanTimeFrame(LEdge *edge, int iFrame) {
		...
		// log an error message
		void logError(const char *strMessage, const char *strFile, int iLine);
		...
		inline int newLWERHistoryItem(int iPrev, unsigned char iAlignmentEvent, LEdge *edge, int iReference) {
		...
		inline void deleteLWERHistoryItem(int iHistoryItem) {
		...
		// activates a token
		inline void activateToken(LWERToken *tokenPrev, LEdge *edge, unsigned char iAlignmentEvent) {
		...
		inline static void reset(LatticeDepth *latticeDepth) {
		...
		// accumulate
		inline static void add(LatticeDepth *latticeDepth1, LatticeDepth *latticeDepth2) {
		...
		// print 
		inline static void print(LatticeDepth *latticeDepth) {
		...
		// reset the object values
		inline static void reset(LatticeWER *latticeWER) {
		...
		// accumulate 
		inline static void add(LatticeWER *latticeWER1, LatticeWER *latticeWER2) {
			...
			latticeWER1->fWER = (latticeWER1->iErrors*100.0)/((float)latticeWER1->iWordsReference);
		}
		
		// print 
		inline static void print(LatticeWER *latticeWER) {
		...
		inline float edgeLogLikelihood(LEdge *edge) {
		
			float fLikelihood = 0.0;
			fLikelihood += (edge->lexUnit->fInsertionPenalty+edge->fScoreAM)*m_fAMScalingFactor;
		...
		inline float edgeLogPP(LEdge *edge) {
		
			return log(edge->fPP);
		}
		
		// set scaling factors
		inline void setScalingFactors(float fScaleAM, float fScaleLM) {
		...
		bool addPath(Alignment *alignment, bool bIsBest = false);
		...
		bool computePhoneAccuracy(VLPhoneAlignment &vLPhoneAlignment, bool bSetSilenceToZero = true, 
			bool bSetFillersToZero = true);	
		
		// return the proportion of the reference phone that overlaps with the given hypothesis phone
		inline float getOverlapProportion(LPhoneAlignment *phoneAlignmentRef, LPhoneAlignment *phoneAlignmentHyp) {
		...
		bool hmmMarking(HMMManager *hmmManager);
		...
		inline void shiftLeftContext(unsigned char *iContextLeftSrc, unsigned char iPosition, 
		...
		// note: destination context must be already filled with padding symbols
		inline void shiftRightContext(unsigned char *iContextRightSrc, unsigned char iPosition, 
		...
		// copy context
		inline unsigned char *copyContext(unsigned char *iContext) {
		...
		inline void printContext(unsigned char *iContext) {
		...
		//       word context
		bool attachLMProbabilities(LMManager *lmManager);
