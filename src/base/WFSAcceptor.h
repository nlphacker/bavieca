	...
	public:	
		...
		bool checkTransition(TransitionX *transitionX) {
			...
			// lexical unit
				...
				assert((iLexUnit >=0) && (iLexUnit < m_iLexUnits));
				if ((iLexUnit < 0) || (iLexUnit >= m_iLexUnits)) {
					return false;
				}
			}
			// HMM-state
				...
				assert((iHMMState >= 0) && (iHMMState < m_iHMMStates));		
				if ((iHMMState < 0) && (iHMMState >= m_iHMMStates)) {
					return false;
				}
			}
