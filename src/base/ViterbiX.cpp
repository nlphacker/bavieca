...
Alignment *ViterbiX::processUtterance(VLexUnit &vLexUnitTranscription, bool bMultiplePronunciations, 
	VLexUnit &vLexUnitOptional, float *fFeatures, int iFeatures, double *dUtteranceLikelihood, 
	float *fFeatureVectorsUpdate, int &iErrorCode) {
	
	double dTimeBegin = Time::getTimeMilliseconds();
	
	// estimation properties
	bool bSingleGaussian = m_hmmManager->isSingleGaussian();
	bool bAccumulatorsLogical = m_hmmManager->areAccumulatorsLogical();
	...
	for(int t = iFeatures-1 ; t >= 0 ; --t) {
		...
		if (iStatesLeft == 0) {
			...
			if (lexUnit != NULL) {
				alignment->addLexUnitAlignment(t+1,iFrameEnd,lexUnit);
			}
			iFrameEnd = t;
	...
	if ((iStatesLeft == 1) && (lexUnit != NULL)) {
		alignment->addLexUnitAlignment(0,iFrameEnd,lexUnit);	
	}	
			...
			for(FBEdgeHMM *edge = edgeActiveCurrent[i]->nodePrev->edgePrev ; edge != NULL ; edge = edge->edgeNext) {
				if (t > edge->nodePrev->iDistanceStart) {
					//if (trellis[(t-1)*iEdges+edge->iEdge].dViterbi != -FLT_MAX) {
						//nodeTrellis->dViterbi = NumericalFunctions::logAddition(trellis[(t-1)*iEdges+edge->iEdge].dViterbi,nodeTrellis->dViterbi);
					//}
					if (trellis[(t-1)*iEdges+edge->iEdge].dViterbi != -FLT_MAX) {
