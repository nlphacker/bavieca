...
// constructor
HypothesisLattice::HypothesisLattice(PhoneSet *phoneSet, LexiconManager *lexiconManager, Log *log)
{
	m_phoneSet = phoneSet;
...
void HypothesisLattice::printProperties() {
	...
	float fKB = ((float)(m_iNodes*sizeof(LNode)+m_iEdges*sizeof(LEdge)))/1024.0;

	printf("- lattice properties ---------------------\n");
...
void HypothesisLattice::load(const char *strFile, unsigned char iFormat) {

	if (iFormat == FILE_FORMAT_TEXT) {
		loadTextFormat(strFile);
	} else {
		assert(iFormat == FILE_FORMAT_BINARY);
...
// load the lattice from the given file (text format)
void HypothesisLattice::loadTextFormat(const char *strFile) {
...
void HypothesisLattice::loadBinaryFormat(const char *strFile) {
	...
	if (!isProperty(LATTICE_PROPERTY_VERSION) ||
		...
		EXCEPTION("wrong lattice format: missing properties");
	}
	...
	for(int i=0 ; i < m_iEdges ; ++i) {
		...
		if (!ledge->lexUnit) {
			EXCEPTION("wrong lattice format: unknown lexical unit");
		}
		...
		if (isProperty(LATTICE_PROPERTY_HMMS) || isProperty(LATTICE_PROPERTY_PHONE_ALIGN)) {
			...
			ledge->iPhones = ((char*)&ledge->iPhones)[0];				// HACK for backwards compatibility
			if (ledge->iPhones != ledge->lexUnit->vPhones.size()) {
				EXCEPTION("wrong lattice format: inconsistent number of phones in edge");
			}	
	...
	for(int i=0 ; i < m_iNodes ; ++i) {
		...
		if ((lnode->iFrame >= m_iFrames) || (lnode->iFrame < -1)) {
			EXCEPTION("wrong lattice format: inconsistent node time");
		}
		lnode->edgePrev = NULL;
		lnode->edgeNext = NULL;
	}
	...
	for(int i=0 ; i < m_iNodes ; ++i) {
		if (m_lnodes[i]->edgePrev == NULL) {
			if (m_lnodeInitial != NULL) {
				EXCEPTION("wrong lattice format: multiple initial nodes defined");
			}	
			m_lnodeInitial = m_lnodes[i];
		}
		if (m_lnodes[i]->edgeNext == NULL) {
			if (m_lnodeFinal != NULL) {
				EXCEPTION("wrong lattice format: multiple final nodes defined");
			}	
			m_lnodeFinal = m_lnodes[i];
		}	
	}
	if ((m_lnodeInitial == NULL) || (m_lnodeFinal == NULL)) {
		EXCEPTION("wrong lattice format: no initial/final node");
	}
	if ((m_lnodeInitial->iFrame != -1) || (m_lnodeFinal->iFrame != m_iFrames-1)) {
		EXCEPTION("wrong lattice format: inconsistent initial/final nodes");
	}
	if (m_lnodeInitial == m_lnodeFinal) {
		EXCEPTION("wrong lattice format: initial node and final node are the same");
	}
...
void HypothesisLattice::check() {
	...
	for(int i=0 ; i<m_iNodes ; ++i) {
		if (m_lnodes[i]->bTouched == false) {
			ERROR << "lattice did not pass check";
		}
	}
...
void HypothesisLattice::logError(const char *strMessage) {
...
// log an error message
void HypothesisLattice::logError(const char *strMessage, const char *strFile, int iLine) {

	char strMessageAux[MAX_LOG_MESSAGE_LENGTH+1];
	
	sprintf(strMessageAux,"%s, file: \"%s\" line:",strMessage,strFile,iLine);

	return logError(strMessageAux);
}

// compute forward backward scores
bool HypothesisLattice::computeForwardBackwardScores(float fScalingAM, float fScalingLM) {
...
void HypothesisLattice::forward(LEdge *edge) {
	...
	while(edgePred != NULL) {
		...
		dScoreAcc = NumericalFunctions::logAddition(dScoreAcc,dAux);
...
bool HypothesisLattice::computePosteriorProbabilities() {
	...
	for(LEdge *edge = m_lnodeFinal->edgePrev ; edge != NULL ; edge = edge->edgeNext) {
		dNorm = NumericalFunctions::logAddition(dNorm,edge->dScoreForward+edge->dScoreBackward);
		dAccFin = NumericalFunctions::logAddition(dAccFin,edge->dScoreForward+edge->dScoreBackward);
	}
	for(LEdge *edge = m_lnodeInitial->edgeNext ; edge != NULL ; edge = edge->edgePrev) {
		dAccIni = NumericalFunctions::logAddition(dAccIni,edge->dScoreForward+edge->dScoreBackward);
	}
	//printf("ini sum: %20.8f\n",dAccIni);
	//printf("fin sum: %20.8f\n",dAccFin);
	...
	for(int i=0 ; i < m_iEdges ; ++i) {
		...
		m_ledges[i]->fPP = exp(m_ledges[i]->dScoreForward+m_ledges[i]->dScoreBackward-dNorm);
		//assert(m_ledges[i]->fPP >= 0.0);
		//assert(m_ledges[i]->fPP <= 1.1);
	}	
...
bool HypothesisLattice::computeConfidenceScore(unsigned char iConfidenceMeasure) {
	...
	// from overlapping edges in the lattice that may not overlap between them
		...
		for(int i=0 ; i < m_iEdges ; ++i) {
			if (m_ledges[i]->fConfidence > 0.0) {
				//printf("conf: %f\n",m_ledges[i]->fConfidence);
			}
		}
	...
	// maximum posterior probability of an overlapping edge
		...
		for(MIntLEdge::iterator it = mLexUnitEdges.begin() ; it != mLexUnitEdges.end() ; ++it) {
			...
			do {
				...
				// sanity check
				//assert(edge->fConfidence >= 0);
				//assert(edge->fConfidence <= 1.1);
...
bool HypothesisLattice::hmmMarking(HMMManager *hmmManager) {
	...
	m_iPhoneContextPadding = m_phoneSet->getSize();
	...
	for(int i=0 ; i < m_iEdges ; ++i) {
		...
		unsigned char iContextLeft[iContextSizeMax];
		unsigned char iContextRight[iContextSizeMax];
	...
	for(int i=0 ; i < m_iEdges ; ++i) {
		...
		//delete [] m_ledges[i]->iContextLeft;
		//delete [] m_ledges[i]->iContextRight;
	}	
...
LatticeWER *HypothesisLattice::computeWER(VLexUnit &vLexUnitReference, Mappings *mappings) {
	...
	while(m_iActiveTokensCurrent > 0) {
		...
		for(int i=0 ; ((i < m_iMaxActiveTokens) && (iTokensToProcess > 0)) ; ++i) {
			...
			if ((m_tokenCurrent[i].node == m_lnodeFinal) && (m_tokenCurrent[i].iReference == vLexUnitReference.size()-1)) {
			...
			// propagate tokens through the successor nodes
				...
				for(LEdge *edge = m_tokenCurrent[i].node->edgeNext ; edge != NULL ; edge = edge->edgePrev) {
					...
					if (m_tokenCurrent[i].iReference+1 < vLexUnitReference.size()) {
						// deletion
						activateToken(&m_tokenCurrent[i],NULL,ALIGNMENT_EVENT_DELETION);
					}
					// substitution/correct
					if ((m_tokenCurrent[i].node != m_lnodeFinal) && (m_tokenCurrent[i].iReference+1 < vLexUnitReference.size())) {
						// no mappings
						if (mappings == NULL) {	
	...
	latticeWER->fWER = ((float)latticeWER->iErrors*100.0)/((float)latticeWER->iWordsReference);	
...
//       word context
bool HypothesisLattice::attachLMProbabilities(LMManager *lmManager) {
...
bool HypothesisLattice::clearBestPath() {
...
bool HypothesisLattice::addPath(Alignment *alignment, bool bIsBest) {
	...
	for(VWordAlignment::iterator it = vWordAlignment->begin() ; it != vWordAlignment->end() ; ++it) {
		LEdge *edge = newEdge((*it)->iFrameBegin,(*it)->iFrameEnd,
			m_lexiconManager->getLexUnitPron((*it)->iLexUnitPron),-FLT_MAX,-FLT_MAX);
		edge->nodePrev = node;
...
double HypothesisLattice::getLikelihood() {
	...
	for(LEdge *edge = m_lnodeFinal->edgePrev ; edge != NULL ; edge = edge->edgeNext) {
		dLikelihood = NumericalFunctions::logAddition(dLikelihood,edge->dScoreForward);	
	}
...
bool HypothesisLattice::computePhoneAccuracy(VLPhoneAlignment &vLPhoneAlignment, bool bSetSilenceToZero, 
	...
	for(int i=0 ; i < m_iEdges ; ++i) {
		...
		for(int iPhone = 0 ; iPhone < edge->iPhones ; ++iPhone) {
			...
			for(VLPhoneAlignment::iterator it = vLPhoneAlignment.begin() ; it != vLPhoneAlignment.end() ; ++it) {
				...
				if (fOverlap > 0.0) {
					...
					if (phoneAlignment->iPhone == (*it)->iPhone) {
						fAccuracy = -1.0 + 2.0*fOverlap;
					}
					// different phone
					else {
						fAccuracy = -1.0 + fOverlap;	
					}
			...
			/*if ((fAccuracyMax < -1.0) || (fAccuracyMax > 1.0)) {
				exit(-1);
			}*/
